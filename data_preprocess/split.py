...
def split_to_train_val_test(args,input_labels_dir_path, output_lst_dir_path, output_lst_file_path,
    ...
    train_ratio, val_ratio, _ = [0.1*int(item) for item in  args.train_val_test_ratio.split(":")]
    ...
    with open(output_lst_file_path, 'w', encoding='utf-8') as f0:  # f0是用来写入所有的label的
        index = 0
        with open(os.path.join(output_lst_dir_path , 'im2latex_train_filter.lst'), 'w', encoding='utf-8') as f1:  #f1用于写入img(分开了train, dev, test)来写入
            for i in tqdm(range(train_num)):
                ...
                if image_name in images_name_list:
                    ...
                    with open(os.path.join(input_labels_dir_path , train_label_name), 'r', encoding='utf-8') as f2:
                        line = f2.read()  # '\\left[ \\pi , \\frac { 4 \\pi } { 3 } \\right]' latex公式
                        f0.write(line + '\n')
                    index += 1

        with open(os.path.join(output_lst_dir_path , 'im2latex_validate_filter.lst'), 'w', encoding='utf-8') as f1:
            for i in tqdm(range(val_num)):
                ...
                if image_name in images_name_list:
                    ...
                    with open(os.path.join(input_labels_dir_path , val_label_name), 'r', encoding='utf-8') as f2:
                        line = f2.read()
                        f0.write(line + '\n')
                    index += 1

        with open(os.path.join(output_lst_dir_path , 'im2latex_test_filter.lst'), 'w', encoding='utf-8') as f1:
            for i in tqdm(range(test_num)):
                ...
                if image_name in images_name_list:
                    ...
                    with open(os.path.join(input_labels_dir_path , test_label_name), 'r', encoding='utf-8') as f2:
                        line = f2.read()
                        f0.write(line + '\n')
                    index += 1